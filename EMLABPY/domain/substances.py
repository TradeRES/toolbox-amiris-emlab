...
class Substance(ImportObject):
    ...
    def add_parameter_value(self, reps, parameter_name, parameter_value, alternative):
        ...
        elif parameter_name == 'nextPrice':
            self.nextPrice = parameter_value
    ...
    def get_price_for_next_tick(self, tick, substance):
        if tick == 0 :
            ...
            self.nextPrice.append(np.interp(tick, xp, fp))
            return
        else:
            while len(self.nextPrice) <= tick:
                last_value = self.nextPrice[-1]
                random_number = random.triangular(-1, 0, 1) # TODO this was (-1,  1, 0 ) for competes integration
                if random_number < 0:
                    self.nextPrice.append(last_value * (self.trend.top + (random_number * (self.trend.top - self.trend.min))))
                else:
                    self.nextPrice.append(last_value * (self.trend.top + (random_number * (self.trend.max - self.trend.top))))
        return
    ...
    def initializeGeometricTrendRegression(self):
        ...
        for index, simulatedPrices in enumerate(self.nextPrice):
