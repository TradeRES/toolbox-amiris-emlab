...
class Investmentdecision(DefaultModule):
    ...
    def act(self):
        for powerplant in self.reps.get_candidate_power_plants_by_owner(self.agent):
            print(powerplant)
            print(F"{self.agent} invests in technology at tick {self.current_tick}")



            #calculateDiscountedValues()
