...
class Amiris:
    ...
    OPERATOR_AGENTS = [renewable_type_name, conventional_type_name, biogas_type_name]
...
def create_AMIRIS_protobuf(dmgr, config, params):
    ...
    if 'output' in args:
        ...
        if file_extension != ".pb":
            raise_and_log_critical_error("Provide a path to a config.pb file including the '.pb' extension. "
...
def get_region_of_all_exchanges(scenario: dict) -> dict:
    ...
    try:
        exchanges = [{exchange['Id']: exchange['Attributes']['Region']} for exchange in scenario['Agents'] if exchange['Type']=='EnergyExchange']
    except KeyError:
        exchanges = [{exchange['Id']: "Region"} for exchange in scenario['Agents'] if exchange['Type']=='EnergyExchange']
    output = {}
...
def compile_price_exchange_file(dmgr, config, params):
    ...
    parsed_data = pd.read_csv(filepath_or_buffer=folder_name+"/"+agent+".csv",
    ...
    parsed_data[index_label] = [time.replace("_", " ") for time in convert_fame_time_to_datetime(parsed_data[index_label])]
    ...
    file_name = "{}_{}_{}_{}_{}.csv".format(str(year),
                                            run_name,
                                            run_id,
                                            institution,
                                            value)

    output.to_csv(path_or_buf=output_folder_path + file_name, sep=config['user']['global']['output']['csvSeparator'], index=None)
...
def get_installed_power_by_technology(scenario: dict, single_file_mode=False) -> pd.DataFrame:
    ...
    if single_file_mode:
        ...
        for technology in installed_power_by_type:
            installed_power_by_type[technology] = sum([sum(list(entry.values())) for entry in installed_power_by_type[technology]])
        installed_power_by_type = pd.DataFrame(installed_power_by_type.items(), index=None, columns=output_labels_no_region)
...
def aggregate_technologies(agents_with_matched_regions, output_labels) -> pd.DataFrame:
    ...
    installed_power_by_type = pd.DataFrame(agents_with_matched_regions).stack().reset_index().set_axis(output_labels, axis=1)
    return installed_power_by_type
        ...
        if 'Attributes' in agent:
            if Amiris.capacitiy_name in agent['Attributes']:
                ...
                if Amiris.energy_carrier_name in agent['Attributes']:
                    ...
                    raise_and_log_critical_error("Found no eligible technology for agent '{}' with id '{}'".format(agent, agent['Id']))
...
def get_agent_with_id_and_type_and_region_from(scenario: dict, agent_type_name: str) -> list:
    ...
    try:
        return [{'Id': agent['Id'], 'Type': agent_type_name, 'Region': get_from_dict_or_default(Amiris.region_name, agent['Attributes'], None)} for agent in scenario['Agents'] if agent['Type'] == agent_type_name]
    except KeyError:
...
def get_region_of_connected_exchange(exchanges: list, contract: dict) -> str:
    ...
    _NOT_EXACTLY_ONE_EXCHANGE = "Your criteria match {} exchanges. Expected to find single exchange in " \
                                "Contract {}."
...
def get_contract_from_sender_with_product(sender_id: int, product_name: str, contracts: list) -> dict:
    ...
    """
    _NOT_EXACTLY_ONE_CONTRACT = "Your criteria match {} contracts. Expected to find single contract with " \
                                "`SenderId` '{}' and `ProductName` '{}'."

    contract = [contract for contract in contracts if contract['ProductName'] == product_name if sender_id in to_list(contract['SenderId'])]
...
def plot_lines(dmgr, config, params):
    ...
    parsed_data = pd.read_csv(filepath_or_buffer=folder_name + "/" + agent_name + ".csv",
                              sep=config['user']['global']['output']['csvSeparator'],
                              )
    ...
    while end < grouped_data[time_col_name].max():
        ...
        for subplot in args['plotData']:
            ...
            plot_data.plot(ax=a[s], title=subplot['title'], ylim=ylim, label="Test", legend=0)
        ...
        if get_from_dict_or_default('watermark', args, False):
            f.text(0.01, 0.01, config['user']['global']['pbFile'], fontsize=8, c="grey")
        time_string = datetime.strftime(begin, format="%Y-%m-%d_%H%M")
        plt.savefig(fname=output_path + agent_name + "_" + time_string[:-5] + "_" + str(window) + "_" + str(x_dim) + "X" + str(y_dim) + ".png",
                    dpi=args['figure']['dpi'])
...
def convert_to_datetime(args, parsed_data):
    ...
    parsed_data[target_name] = [datetime.strptime(FameTime.convert_fame_time_step_to_datetime(x), DATE_FORMAT) for x in
...
def analyse_time_series(dmgr, config, params):
    ...
    for evaluation in args['analysis']:
        ...
        for agent_id, data in grouped_data_by_agentid.items():
            ...
            write_to_file(file, "`{}` for Agent with Id `{}`".format(column, str(agent_id)), args["consoleOutput"])
            write_to_file(file, "{}".format(selected_data.describe().reset_index().to_string(header=None, index=None)), args["consoleOutput"])
            write_to_file(file, "`{}` occurances: {}".format(evaluation["countNumberOfValue"], len(selected_data[selected_data == evaluation["countNumberOfValue"]])), args["consoleOutput"])
...
def plot_price_duration_curve(dmgr, config, params):
    ...
    if 'backtestAgainst' in args:
        backtest_data = pd.read_csv(filepath_or_buffer=args['backtestAgainst'], sep=config['user']['global']['output']['csvSeparator'], names=[args['time']['originName'], 'Backtest'])
    ...
    f.legend(lines, labels, loc='lower center', ncol=len(labels), bbox_to_anchor=(0.5, 0.06))
    ...
    if get_from_dict_or_default('watermark', args, False):
        f.text(0.01, 0.01, config['user']['global']['pbFile'], fontsize=8, c="grey")
    plt.savefig(fname=output_path + agent_name + "_" + args["column"] + "_price_duration.png", dpi=args['figure']['dpi'])
    plt.close('all')
    sns.reset_orig()
...
def plot_multiple_lines(dmgr, config, params):
    ...
    if 'backtestAgainst' in args:
        backtest_data = pd.read_csv(filepath_or_buffer=args['backtestAgainst'], sep=config['user']['global']['output']['csvSeparator'], names=[args['time']['originName'], 'Historical prices'])
        backtest_data = convert_to_datetime(args, backtest_data)
    ...
    while end < grouped_data[time_col_name].max():
        ...
        for subplot in args['plotData']:
            ...
            plot_data.plot(ax=a[s], title=subplot['title'], ylim=(y_min, y_max), label="Test", legend=0)
...
def plot_heat_map(dmgr, config, params):
    ...
    f, a = plt.subplots(nrows=1, ncols=1, figsize=(args['figure']['width'], args['figure']['height']),)
    ...
    plt.imshow(plot_data, aspect='auto', origin='lower', cmap=args["cmap"], interpolation='nearest',
               vmin=vmin, vmax=vmax)
    ...
    plt.savefig(fname=output_path + agent_name + "_" + args["column"] + "_heat_map.png", dpi=args['figure']['dpi'])
    plt.close('all')
...
def compile_installed_capacities_exchange_file(dmgr, config, params):
    ...
    installed_capacities.rename({"InstalledCapacitiesInMW": "Installed Capacities In MW"}, axis=1, inplace=True)
    ...
    installed_capacities.to_csv(path_or_buf=output_folder_path + file_name, sep=config['user']['global']['output']['csvSeparator'], index=None)
...
def get_energycarrier_name_for(agent_id: int, installed_power_by_energycarrier: dict) -> str:
    ...
    for energy_carrier_name, energy_carrier_group in installed_power_by_energycarrier.items():
        for plant in energy_carrier_group:
            for plant_id, _ in plant.items():
                if agent_id == plant_id:
                    return energy_carrier_name

    raise_and_log_critical_error("Did not find agent with `Id` {} in list of installed powers by energycarrier.".format(agent_id))
...
def compile_awarded_power_exchange_file(dmgr, config, params):
    ...
    for agent in agents:
        ...
        agent_data[Amiris.time_step] = [time.replace("_", " ") for time in convert_fame_time_to_datetime(agent_data[Amiris.time_step])]
    ...
    for region, agents in regions.items():
        ...
        output = pd.DataFrame(*[temp]).stack().reset_index().set_axis(["Time", "Energy Carrier", "Awarded Power In MWh"], axis=1)
        output['Region'] = region
        main_output = pd.concat([main_output, output.reindex(["Time", "Region", "Energy Carrier", "Awarded Power In MWh"], axis=1)])
    ...
    main_output.to_csv(path_or_buf=output_folder_path + file_name, sep=config['user']['global']['output']['csvSeparator'], index=None)
...
def get_operator_id_for(agent: dict, contracts: list) -> int:
    ...
    assert agent['Type'] == Amiris.conventional_type_name, "Can only find operator_id for `PredefinedPlantBuilder`, but not provided `{}`.".format(agent['Type'])
...
def compile_transfer_capacity_exchange_file(dmgr, config, params):
    ...
    parsed_data["UsedTransferCapacityInMWH"] = parsed_data["AvailableTransferCapacityInMWH"] - parsed_data["UsedTransferCapacityInMWH"]
    ...
    parsed_data.to_csv(path_or_buf=output_folder_path + file_name, sep=config['user']['global']['output']['csvSeparator'], index=None)
...
def compile_powerplant_generation_exchange_file(dmgr, config, params):
    ...
    for unique_id in parsed_data['Id'].unique():
        ...
        tmp_data[ramp_up_name] = tmp_data.apply(lambda row: (agents_with_installed_power[int(powerplant_id)] - row[power_name]), axis=1)
    ...
    if get_from_dict_or_default('save_as_hdf5', args, False):
        output_data.to_hdf(output_folder_path + file_name.split(".")[0] + ".hdf5", key=value, mode="w", complevel=9)
    else:
        output_data.to_csv(path_or_buf=output_folder_path + file_name, sep=config['user']['global']['output']['csvSeparator'], index=None)
...
def compare_price_duration_curves(dmgr, config, params):
    ...
    linestyles = {"DLR": "dotted",
                  "KIT": "solid"}

    regions_to_plot = get_from_dict_or_default('limit_to_regions', args, None)

    for item in args['data']:
        parsed_data = pd.read_csv(filepath_or_buffer=item['file_path'], sep=config['user']['global']['output']['csvSeparator'])
        ...
        for i, col in enumerate(plot_data.columns):
            ...
            sorted_data.plot(ax=a, legend=0, use_index=False, color=color, linestyle=linestyles[item["origin"]])
    ...
    plt.savefig(fname=output_path + str(filename) + "_Price_Duration.png", dpi=args['figure']['dpi'])
    plt.close('all')
    sns.reset_orig()
...
def compare_heat_maps(dmgr, config, params):
    ...
    f, a = plt.subplots(nrows=x_dim, ncols=y_dim, figsize=(args['figure']['width'], args['figure']['height']),)
    ...
    for idx, item in enumerate(args['data']):
        ...
        im = a[s].imshow(plot_data, aspect='auto', origin='lower', cmap=args["cmap"], interpolation='nearest', vmin=vmin, vmax=vmax)
        divider = make_axes_locatable(a[s])
        cax = divider.append_axes("right", size="1%", pad=0.05)
        cbar = plt.colorbar(im, cax=cax, pad=.02)
    ...
    if normalize:
        plt.savefig(fname=output_path + str(filename) + "_Heat_Map_normalized.png", dpi=args['figure']['dpi'])
    else:
        plt.savefig(fname=output_path + str(filename) + "_Heat_Map.png", dpi=args['figure']['dpi'])
    plt.close('all')
...
def get_markup_for(id: int, scenario: dict, type: str) -> float:
    ...
    if len(trader_agent) != 1:
        raise_and_log_critical_error("Found {} occurences for trader with id `{}`. Expected 1 instead.".format(len(trader_agent), id))
...
def compile_markup_exchange_file(dmgr, config, params):
    ...
    export_columns = ["Year", "Id", "Markup In EUR Per MWh", "Markdown In EUR Per MWh"]
